    ...
    match cli.command {
        ...
        Commands::Program { action } => match action {
            ...
            ProgramAction::Add {
                ...
                platform,
                url,
            } => commands::programs::add(project, name, platform, url).await?,
        ...
        Commands::Scope { action } => match action {
            ...
            ScopeAction::Add {
                ...
                r#type,
                out_of_scope,
            } => commands::scope::add(program, target, r#type, out_of_scope).await?,
        ...
        Commands::Findings { action } => match action {
            FindingAction::List {
                ...
                status,
                unassigned,
            } => commands::findings::list(program, severity, status, unassigned).await?,
            ...
            FindingAction::Report { id, platform } => {
                commands::findings::report(id, platform).await?
            }
        },
        Commands::Monitor { action } => match action {
            ...
            MonitorAction::SetInterval { program, hours } => {
                commands::monitor::set_interval(program, hours).await?
            }
        },
    }
