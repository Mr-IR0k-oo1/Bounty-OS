...
use bountyos_analyzer::{
    analysis::{c::find_unbounded_stack_copies, rules::detect_buffer_overflow},
    cwe, Correlator, OllamaAnalyzer, RuleAnalyzer, VulnerabilityAnalyzer,
};
use reqwest::Client;
...
"#;
            ...
            let llm = OllamaAnalyzer::new("http://localhost:11434", "vulnllm-r-secure");

            let rules = RuleAnalyzer::new();

            let llm_result = llm.analyze(source).await?;
            let rule_result = rules.analyze(source).await?;

            let result = Correlator::correlate(&llm_result, &rule_result);
